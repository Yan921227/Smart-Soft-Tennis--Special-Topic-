...
def _angle(kps,a,b,c):
    ...
    return math.acos(max(-1,min(1,dot/(math.hypot(*v1)*math.hypot(*v2)+1e-6))))
...
def build_feat(kps):
    ...
    ankle_min = min(kps[31]['y'], kps[32]['y'])
    hip_to_ankle = ankle_min - hip_y
...
for folder in sorted(ROOT.iterdir()):
    ...
    for i,jf in enumerate(sorted(folder.glob('*.json'))):
        ...
        kps = extract_pose(json.load(open(jf,'r',encoding='utf-8')))
...
for attempt in range(1, MAX_RETRY+1):
    ...
    if attempt == MAX_RETRY:
        sys.exit("❌ 連抽 {MAX_RETRY} 次仍缺類別，請增加資料或手動分割")
...
joblib.dump({'model': clf, 'label_encoder': le},
            'softtennis_pose_random_forest.pkl', compress=3)
